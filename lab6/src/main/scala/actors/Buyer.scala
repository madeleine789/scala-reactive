...
import actors.search.AuctionSearch.{NoResults, SearchResults, Search}
...
class Buyer(target: String, maxPrice: Int) extends Actor with ActorLogging {
  ...
  val masterSearch = context.actorSelection("akka.tcp://AuctionSystem@127.0.0.1:2551/user/MasterSearch")
  //auctionSearch ! Search(target)
  masterSearch ! Search(target)
