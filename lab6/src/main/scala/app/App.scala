...
import actors.search.{MasterSearch, AuctionSearch}
...
object App {
  ...
  def main(args: Array[String]){
    ...
    sellers.foreach { key =>
      val seller = system.actorOf(Props(classOf[Seller], key._2, true), s"Seller_${key._1}")
    }
    //system.actorOf(Props[AuctionSearch], "AuctionSearch")
    val masterSearch = system.actorOf(Props(new MasterSearch(2)), "MasterSearch")
